...
컴포넌트는 텍스트와 이미지를 지칭하여, JSON KVP(Key-Value Pair) 로 표현한다. ::
...
-  ``텍스트`` 는 반드시 ``txt-`` 로 시작해야 한다.
-  ``이미지`` 는 반드시 ``img-`` 로 시작해야 한다.

엔진은 <Value>를 <Key> 방식으로 그린다. <Key>는 반드시 ``#style`` 에서 참조가능해야 하며, 찾을 수 없을 경우 해당 컴포넌트는 그려지지 않는다.
...
각 컴포넌트-스타일(컴포넌트를 그리기 위해 정의한 스타일)은 1차원 JSON 형식으로 표현되며 `CSS<https://www.w3schools.com/css/default.asp>`_ 와 동일한 Key-Value를 가진다. 
...
후가공은 Snapfy엔진 작업 완료 후 이미지툴 엔진에서 이루어진다. ::
